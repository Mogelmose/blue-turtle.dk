...
export async function POST(request) {
  ...
  try {
      ...
      return NextResponse.json(
        {
          success: false,
          error:
            'Den nye adgangskode skal være forskellig fra den nuværende adgangskode',
        },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json(
      { success: true, message: 'Adgangskode opdateret succesfuldt' },
      { status: 200 }
    );
  } catch (error) {
    ...
    if (error instanceof z.ZodError) {
      ...
      return NextResponse.json(
        { success: false, error: firstError?.message || 'Ugyldige data.' },
        { status: 400 }
      );
    }
