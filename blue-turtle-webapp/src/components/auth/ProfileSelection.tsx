...
const ProfileSelection: React.FC<ProfileSelectionProps> = ({ profiles, handleProfileClick, fejlbesked }) => {
  return (
    <div className="w-full max-w-4xl text-center">
      ...
      {fejlbesked && (
        <div className="mb-4 p-3 text-sm font-medium text-red-700 bg-red-100 dark:bg-red-900/30 dark:text-red-300 rounded-lg" role="alert">
          {fejlbesked}
        </div>
      )}
      ...
      {profiles.length > 0 ? (
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 md:gap-6">
          {profiles.map((profile) => (
            ...
            >
              <div className="relative w-24 h-24 md:w-28 md:h-28 rounded-full overflow-hidden border-4 border-light-border dark:border-dark-border group-hover:border-light-primary dark:group-hover:border-dark-primary transition-colors duration-300">
                <Image
                  src={profile.img}
